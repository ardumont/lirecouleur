...
###################################################################################
mots_ent = ['absent', 'abstinent', 'accent', 'accident', 'adhérent', 'adjacent',
...
verbes_enter = ['absenter', 'accidenter', 'agrémenter', 'alimenter', 'apparenter',
...
def regle_nc_ai_final(mot, pos_mot):
    possibles = ['balai', 'brai', 'chai', 'déblai', 'délai', 'essai', 'frai', 'geai', 'lai', 'mai',
...
def regle_s_final(mot, __pos_mot):
    ...
    'tétanos', 'os',
    'alias', 'atlas', 'hélas', 'madras', 'sensas', 'tapas', 'trias', 'vasistas', 'hypocras',
    'gambas', 'as',
    'biceps', 'quadriceps', 'chips', 'relaps', 'forceps', 'schnaps', 'laps', 'oups', 'triceps', 'princeps', 'tricératops']
...
autom = {
    ...
    'a' : [['u', 'il', 'in', 'nc_ai_fin', 'ai_fin', 'i', 'n', 'm', 'nm', 'y_except', 'y'],
            {'n':[{'+':r"n[bcçdfgjklmpqrstvwxz]"}, 'a~', 2],
            'm':[{'+':r"m[mbp]"}, 'a~', 2],  # # règle du m devant m, b, p
            'nm':[{'+':r"n(s?)$"}, 'a~', 2],
            'y_except':[{'-':r"(^b|cob|cip)", '+':r"y"}, 'a', 1],  # # exception : baye, cobaye
            'y':[{'+':r"y"}, 'e^_comp', 1],
            'u':[{'+':r"u"}, 'o_comp', 2],
            'il':[{'+':r"il($|l)"}, 'a', 1],
            'in':[{'+':r"i[nm]([bcçdfghjklnmpqrstvwxz]|$)"}, 'e~', 3],  # # toute succession 'ain' 'aim' suivie d'une consonne ou d'une fin de mot
    ...
    'c' : [['eiy', 'choeur_1', 'choeur_2', 'chor', 'psycho', 'brachio', 'cheo', 'chest', 'chiro', 'chlo_chlam', 'chr',
            ...
            'cciey':[{'+':r"c[eiyéèêëîï]"}, 'k', 1],  # # accident, accepter, coccyx
 ...
 # # + tous les *nc sauf "onc" et "donc"
    ...
    'd' : [['d', 'aujourdhui', 'disole', 'except', 'dmuet', 'apostrophe'],
            {'d':[{'+':r"d"}, 'd', 2],
            'except':[{'-':r"(aï|oue)", '+':r"(s?)$"}, 'd', 1],  # # aïd, caïd, oued
    ...
    'e' : [['conj_v_ier', 'uient', 'ien', 'ien_2', 'een', 'except_en_1', 'except_en_2', '_ent', 'clef', 'hier', 'adv_emment_fin',
            ...
            'et', 'cet', 't_final', 'eclm_final', 'est_1', 'est_2', 'es_1', 'es_2', 'drz_final', 'n', 'adv_emment_a', 'femme', 'lemme', 'em_gene', 'nm', 'tclesmesdes',
            ...
            'n':[{'+':r"n[bcçdfghjklmpqrstvwxz]"}, 'a~', 2],
            ...
            'em_gene':[{'+':r"m[bcçdfghjklmnpqrstvwxz]"}, 'a~', 2],  # # 'em' cas général => son [a~]
            ...
            'except_en_1':[{'-':r"exam|mino|édu", '+':r"n(s?)$"}, 'e~', 2],  # # exceptions des mots où le 'en' final se prononce [e~] (héritage latin)
            'except_en_2':[{'-':r"[ao]ï", '+':r"n(s?)$"}, 'e~', 2],  # # païen, hawaïen, tolstoïen
            'een':[{'-':r"é", '+':r"n(s?)$"}, 'e~', 2],  # # les mots qui se terminent par 'éen'
            'ien':[{'-':r"[bcdlmrstvh]i", '+':r"n([bcçdfghjklpqrstvwxz]|$)"}, 'e~', 2],  # # certains mots avec 'ien' => son [e~]
            'ien_2':[{'-':r"ï", '+':r"n([bcçdfghjklpqrstvwxz]|$)"}, 'e~', 2],  # # certains mots avec 'ien' => son [e~]
            'nm':[{'+':r"[nm]$"}, 'a~', 2],
            ...
            'in':[{'+':r"i[nm]([bcçdfghjklnmpqrstvwxz]|$)"}, 'e~', 3],  # # toute succession 'ein' 'eim' suivie d'une consonne ou d'une fin de mot
            ...
            'eur':[{'+':r"ur"}, 'x', 2],
            'eu':[{'+':r"u"}, 'x', 2],
            'eu_accent_circ':[{'+':r"û"}, 'x^', 2],
            'est_1':[{'-':r"^", '+':r"st$"}, 'e^_comp', 3],
            'es_1':[{'-':r"^", '+':r"s$"}, 'e^_comp', 2],
            'est_2':[{'-':r"@", '+':r"st$"}, 'e^_comp', 3],
            'es_2':[{'-':r"@", '+':r"s$"}, 'e^_comp', 2],
            'et':[{'-':r"^", '+':r"t$"}, 'e_comp', 2],
            'eil':[{'+':r"il"}, 'e^_comp', 1],
            'y':[{'+':r"y[aeiouéèêààäôâ]"}, 'e^_comp', 1],
            'iy':[{'+':r"[iy]"}, 'e^_comp', 2],
            ...
            'reqquechose':[{'-':r"r", '+':r"[bcçdfghjklmnpqrstvwxz](h|l|r)"}, 'q', 1],  # # re-quelque chose : le e se prononce 'e'
            'dessus_dessous':[{'-':r"d", '+':r"ss(o?)us"}, 'q', 1],  # # dessus, dessous : 'e' = e
            '2consonnes':[{'+':r"[bcçdfghjklmnpqrstvwxz]{2}"}, 'e^_comp', 1],  # # e suivi de 2 consonnes se prononce è
            'e_deb':[{'-':r"^"}, 'q', 1],  # # par défaut, un 'e' en début de mot se prononce [q]
            'abbaye':[{'-':r"abbay", '+':r"(s?)$"}, '#', 1],  # # ben oui...
            'e_muet':[{'-':r"[aeiouéèêà]", '+':r"(s?)$"}, '#', 1],  # # un e suivi éventuellement d'un 's' et précédé d'une voyelle ou d'un 'g' ex. : pie, geai
            'jean':[{'-':r"j", '+':r"an"}, '#', 1],  # # jean
            'ge':[{'-':r"g", '+':r"[aouàäôâ]"}, '#', 1],  # # un e précédé d'un 'g' et suivi d'une voyelle ex. : cageot
            'eoi':[{'+':r"oi"}, '#', 1],  # # un e suivi de 'oi' ex. : asseoir
            'e_caduc':[{'-':r"[bcçdfghjklmnpqrstvwxzy]", '+':r"(s?)$"}, 'q_caduc', 1],  # # un e suivi éventuellement d'un 's' et précédé d'une consonne ex. : correctes
            '*':[{}, 'q', 1],
            '@':['', 'q_caduc', 1]
            }],
    'é' : [[],
    ...
    'f' : [['f', 'oeufs'],
            {'f':[{'+':r"f"}, 'f', 2],
             'oeufs':[{'-':r"(oeu|œu)", '+':r"s"}, '#', 1],  # # oeufs et boeufs
             '*':[{}, 'f', 1]}],
    'g' : [['g', 'ao', 'eiy', 'aiguille', 'u_consonne', 'u', 'n', 'vingt', 'g_muet_oin',
            ...
            'n':[{'+':r"n"}, 'n~', 2],
            'ao':[{'+':r"a|o"}, 'g', 1],
            'eiy':[{'+':r"[eéèêëïiy]"}, 'z^_g', 1],  # # un 'g' suivi de e,i,y se prononce [z^]
            ...
            'u_consonne':[{'+':r"u[bcçdfghjklmnpqrstvwxz]"}, 'g', 1],  # # gu suivi d'une consonne se prononce [g][y]
            'u':[{'+':r"u"}, 'g_u', 2],
            '*':[{}, 'g', 1]}],
    'h' : [[],
            {'*':[{}, '#', 1]}],
    'i' : [['ing', 'n', 'm', 'nm', 'prec_2cons', 'lldeb', 'vill', 'mill', 'tranquille',
            ...
            {'ing':[{'-':r"[bcçdfghjklmnpqrstvwxz]", '+':r"ng$"}, 'i', 1],
            'n':[{'+':r"n[bcçdfghjklmpqrstvwxz]"}, 'e~', 2],
            'm':[{'+':r"m[bcçdfghjklnpqrstvwxz]"}, 'e~', 2],
            'nm':[{'+':r"[n|m]$"}, 'e~', 2],
            ...
            'ill':[{'+':r"ll", '-':r"[bcçdfghjklmnpqrstvwxz](u?)"}, 'i', 1],  # # précédé éventuellement d'un u et d'une consonne, donne le son [i]
    ...
    'n' : [['ing', 'n', 'ment', 'urent', 'irent', 'erent', 'ent', 'nisole', 'apostrophe'],
            ...
            'erent':[{'-':r"ère", '+':r"t$"}, 'verb_3p', 2],  # # verbes avec terminaisons en -èrent
    ...
    'o' : [['in', 'oignon', 'i', 'tomn', 'monsieur', 'n', 'm', 'nm', 'y1', 'y2', 'u', 'o', 'oe_0',
            ...
            'i':[{'+':r"(i|î)"}, 'wa', 2],
            'u':[{'+':r"[uwûù]"}, 'u', 2],  # # son [u] : clou, clown
            'tomn':[{'-':r"t", '+':r"mn"}, 'o', 1],  # # regle spécifique pour 'automne' et ses dérivés
            'monsieur':[{'-':r"m", '+':r"nsieur"}, 'q', 2],
            'n':[{'+':r"n[bcçdfgjklmpqrstvwxz]"}, 'o~', 2],
            'm':[{'+':r"m[bcçdfgjklpqrstvwxz]"}, 'o~', 2],  # # toute consonne sauf le m
            'nm':[{'+':r"[nm]$"}, 'o~', 2],
            'y1':[{'+':r"y$"}, 'wa', 2],
            'y2':[{'+':r"y"}, 'wa', 1],
    ...
    's' : [['sch', 'h', 's_final', 'parasit', 'para', 'mars', 's', 'z', 'sisole', 'smuet', 'apostrophe'],
            ...
            'z':[{'-':r"[aeiyouéèàüûùëöêîô]", '+':r"[aeiyouéèàüûùëöêîô]"}, 'z_s', 1],  # # un s entre 2 voyelles se prononce [z]
    ...
    'u' : [['um', 'n', 'nm', 'ueil'],
            {'um':[{'-':r"[^aefo]", '+':r"m$"}, 'o', 1],
            'n':[{'+':r"n[bcçdfghjklmpqrstvwxz]"}, 'x~', 2],
            'nm':[{'+':r"[nm]$"}, 'x~', 2],
    ...
    'x' : [['six_dix', 'gz_1', 'gz_2', 'gz_3', 'gz_4', 'gz_5', '_aeox', 'fix', '_ix'],
            ...
            'gz_1':[{'-':r"^", '+':r"[aeiouéèàüëöêîôûù]"}, 'gz', 1],  # # mots qui commencent par un x suivi d'une voyelle
            'gz_2':[{'-':r"^(h?)e", '+':r"[aeiouéèàüëöêîôûù]"}, 'gz', 1],  # # mots qui commencent par un 'ex' ou 'hex' suivi d'une voyelle
            'gz_3':[{'-':r"^coe", '+':r"[aeiouéèàüëöêîôûù]"}, 'gz', 1],  # # mots qui commencent par un 'coex' suivi d'une voyelle
            'gz_4':[{'-':r"^ine", '+':r"[aeiouéèàüëöêîôûù]"}, 'gz', 1],  # # mots qui commencent par un 'inex' suivi d'une voyelle
            'gz_5':[{'-':r"^(p?)rée", '+':r"[aeiouéèàüëöêîôûù]"}, 'gz', 1],  # # mots qui commencent par un 'réex' ou 'préex' suivi d'une voyelle
            '_aeox':[{'-':r"[aeo]"}, 'ks', 1],
            'fix':[{'-':r"fi"}, 'ks', 1],
            '_ix':[{'-':r"(remi|obéli|astéri|héli|phéni|féli)"}, 'ks', 1],
            '*':[{}, 'ks', 1],
            '@':[{}, '#', 1]}],
    'y' : [['m', 'n', 'nm', 'abbaye', 'y_voyelle'],
            {'y_voyelle':[{'+':r"[aeiouéèàüëöêîôûù]"}, 'j', 1],  # # y suivi d'une voyelle donne [j]
...
def post_traitement_o_ouvert_ferme(pp):
    ...
    mots_osse = ['cabosse', 'carabosse', 'carrosse', 'colosse', 'molosse', 'cosse', 'crosse', 'bosse',
